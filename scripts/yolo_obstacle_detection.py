...
CONF_THRESH = 0.45
DIST_STOP_M = 0.50       # full stop if obstacle closer than 50 cm
DIST_CAUTION_M = 1.20    # slow down if closer than 120 cm
DIST_DANGER_SCREEN_M = 0.35  # full-screen overlay if closer than 35 cm


def get_roi_depth_m(depth_mm, x1, y1, x2, y2):
    """Median valid depth (mm->m) inside a bounding box. Returns inf if no valid pixels."""
...
try:
    while True:
        ...
        # Normalized proximity for HUD (0 = far/safe, 1 = very close)
        ...
        cv2.imshow("YOLO Obstacle + Lane Detection", frame)
